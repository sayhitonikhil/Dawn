  ...
  constructor() {
    ...
    this.querySelector("[data-update-coupon]").addEventListener(
      ...
      (event) => {
        ...
        let discountLoadingSectionUpdated = document.querySelector(
          ".discount-loading-section"
        );
        let sideCartDiscountSection = document.getElementById("total-discount");
        discountLoadingSectionUpdated.classList.add("show-loading");
        ...
        fetch(`/discount/${this.val}`)
          ...
          .then((responseText) => {
            console.log("Discount is applied on this");
            document.querySelector(".popup-toolDown.show").classList.remove("show");
            document.querySelector(".previewCart").classList.remove("active-tool");
            discountLoadingSectionUpdated.classList.remove("show-loading");
            sideCartDiscountSection.style.display = "block";
          });
      }
    );
  }
